...
import { signUpSchema, type SignUpForm as SignUpFormType } from "@/shared/lib/Zod/signUpFormSchema";
...
const SignUpForm = () => {
  ...
  const onPressVerify = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isClerkAPIResponseError(err)) {
        setVerificationError(err.errors[0]?.longMessage || "Verification failed. Please try again.");
      } else {

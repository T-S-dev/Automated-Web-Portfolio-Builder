...
import { createOrUpdatePortfolioAction } from "../../actions";
...
const PortfolioForm = ({ className, onCancel, mode = "create" }: PortfolioFormProps) => {
  const router = useRouter();
  const { user } = useUser();
  const [isSubmitting, startSubmitTransition] = useTransition();

  const { handleSubmit, reset } = useFormContext<Portfolio>();

  const onSubmit = (data: Portfolio) => {
    startSubmitTransition(async () => {
      const result = await createOrUpdatePortfolioAction(data);
      if (result.success) {
        toast.success(`Portfolio ${mode === "create" ? "created" : "updated"} successfully!`);
